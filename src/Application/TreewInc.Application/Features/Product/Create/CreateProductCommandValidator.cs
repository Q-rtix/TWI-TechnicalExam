...
public class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
	public CreateProductCommandValidator()
	{
		RuleFor(x => x.Name)
			...
			.Matches("^[a-zA-Z0-9]+$").WithMessage("The name can only contain letters and numbers")
		...
		{
			RuleFor(x => x.Description)
				.Matches("^[a-zA-Z0-9]+$").WithMessage("The description can only contain letters and numbers and must not be empty");
		});
	}
}